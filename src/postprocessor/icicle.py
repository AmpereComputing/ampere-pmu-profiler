...
""" returns icicle figure with L1, L2, L3 and L4 TDA """
...
def get_icicle(input_csv):
    ...
    fig.add_trace(
        go.Icicle(
            ...
            hovertemplate="<b>%{label}</b> <br> Value: %{text}<br>Description: %{customdata}<extra> </extra>",
        )
    )
    fig.update_traces(
        text=TDA.value.round(decimals=2),
        textinfo="label+text",
...
def realign_metrics(df_orig):
    ...
    for level in sorted(df["level"].unique()):
        ...
        for _, row in level_df.iterrows():
            ...
            if total_original > 0:
                # scale children to match parent while keeping ratios
